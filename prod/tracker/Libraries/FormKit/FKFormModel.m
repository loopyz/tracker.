...
- (UIView *)headerWithViewClass:(Class)klass title:(NSString *)title {
    if (klass && [title length] > 0) {
        ...
        if ([headerView conformsToProtocol:@protocol(FKTitleHeaderViewProtocol)]) {
            ...
            headerViewWithProtocol.backgroundColor = [TRColors daysLeftColor];
...
- (void)setTableView:(UITableView *)tableView {
    ...
    tableView.backgroundColor = [TRColors daysLeftColor];
}
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    UIView *view = [[UIView alloc] init];
    view.backgroundColor = [TRColors daysLeftColor];
    return view;
}
